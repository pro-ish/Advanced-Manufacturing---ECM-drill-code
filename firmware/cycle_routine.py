#!/usr/bin/env python3
"""
Simple ECM cycle:

1. Home to upper limit.
2. Pump ON at 80 %.
3. Move down at 0.5 mm/s until bottom limit.
4. Move up at 0.5 mm/s until top limit.
5. Stop pump, then move 2 mm down from home and stop.
"""

import time, sys, signal
import RPi.GPIO as GPIO

from config import (
    HOME_FEED_MM_S,
    STEPS_PER_MM,
    PUMP_DUTY_RUN,   # not used but imported for consistency
)
from motion import MotionController
from pump import PumpController
from safety import SafetyManager

GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)


def cleanup(motion, pump, safety):
    try:
        pump.off()
    except Exception:
        pass
    try:
        motion.set_enabled(False)
    except Exception:
        pass
    try:
        safety.relay_off()
    except Exception:
        pass
    GPIO.cleanup()
    print("[SYS] Clean exit.")


def main():
    print("[SYS] Simple ECM cycle starting…")

    safety = SafetyManager()
    motion = MotionController()
    pump   = PumpController()

    # E-STOP must be released for relay to arm
    print("[SAFETY] Ensure E-STOP released to arm 12 V relay.")
    time.sleep(0.5)
    safety.relay_on()

    # Ctrl+C handler
    def _sigint(_s, _f):
        print("\n[SYS] Ctrl+C → aborting cycle.")
        cleanup(motion, pump, safety)
        sys.exit(0)

    signal.signal(signal.SIGINT, _sigint)

    try:
        # 1) Home to upper limit
        print("[STEP] Homing to upper limit…")
        motion.set_enabled(True)
        motion.home()

        # 2) Pump ON at 100%
        pump_duty = 100
        print(f"[PUMP] ON @ {pump_duty:.0f} % duty.")
        pump.set_duty(pump_duty)
        time.sleep(0.5)

        # 3) Move down at 0.5 mm/s until bottom limit
        feed_mm_s = 0.5
        print(f"[STEP] Moving DOWN @ {feed_mm_s:.2f} mm/s until bottom limit…")
        # Large travel; move_mm will stop early on limit
        motion.move_mm(-100.0, 0.1)   # -100 mm = “big number” + relies on bottom limit

        # 4) Move up at 0.5 mm/s until top limit
        print(f"[STEP] Moving UP @ {feed_mm_s:.2f} mm/s until top limit…")
        motion.move_mm(+100.0, feed_mm_s)   # +100 mm, stops on top limit

        # 5) At home: stop pump, then 2 mm down
        print("[PUMP] OFF.")
        pump.off()
        time.sleep(0.2)

        print("[STEP] Parking 2 mm DOWN from home…")
        motion.move_mm(-2.0, feed_mm_s)

        print("[SYS] Cycle complete.")
        cleanup(motion, pump, safety)

    except Exception as e:
        print(f"[ERR] {e}")
        cleanup(motion, pump, safety)


if __name__ == "__main__":
    main()
